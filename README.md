...
Every iOS push runs `.github/workflows/ios-ci.yml`, which compiles and tests the app on a macOS/Xcode runner and uploads both a simulator build and an unsigned iPhone IPA. The unsigned IPA can be re-signed with Sideloadly/AltStore for testing. A normally installable/TestFlight IPA requires an Apple Developer membership, distribution certificate and provisioning profile configured as GitHub Actions secrets. See `ios/INSTALL-ON-IPHONE.md` for installation and `ios/SIGNING-ASSET-REGISTER.md` for the permanent update identity.
